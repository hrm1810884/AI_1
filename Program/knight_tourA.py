...
class Board:
    def __init__(self, board, x, y, depth, balance, goal):
        self.board = board
        self.knight_x = x
        self.knight_y = y
        self.depth = depth
    ...
    def tryMove(self, x, y):
        ...
        if (self.movable(x, y)):
            ...
            if (self.isnotCONSIDERED(board_next)):
                if(abs(x) == 2):
                    B_next = Board(board_next, knight_x, knight_y, self.depth+1, self.balance+1, self.goal)
                else:
                    B_next = Board(board_next, knight_x, knight_y, self.depth+1, self.balance-1, self.goal)
    ...
    def isnotCONSIDERED(self, board):
        ...
        return self.notin(board) and self.notin(board90) and self.notin(np.rot90(board90)) and self.notin(np.rot90(board, 3)) and self.notin(np.flipud(board)) and self.notin(np.fliplr(board)) and self.notin(np.fliplr(board90)) and self.notin(np.flipud(board90))
    ...
    def cost(self):
        ...
        if(self.movable(-2,-1)):
            access += 1
        if(self.board[2][1] and not self.board[0][0]):
            penalty += 1
        if(self.board[1][2] and not self.board[0][0]):
            penalty += 1
        if(self.board[1][mapSize-3] and not self.board[0][mapSize-1]):
            penalty += 1
        if(self.board[2][mapSize-2] and not self.board[0][mapSize-1]):
            penalty += 1
        if(self.board[mapSize-3][1] and not self.board[mapSize-1][0]):
            penalty += 1
        if(self.board[mapSize-2][2] and not self.board[mapSize-1][0]):
            penalty += 1
        if(self.board[mapSize-3][mapSize-2] and not self.board[mapSize-1][mapSize-1]):
            penalty += 1
        if(self.board[mapSize-2][mapSize-3] and not self.board[mapSize-1][mapSize-1]):
            penalty += 1
        if(not access or penalty >= 2):
            return inf
        return 4.0*mapSize*access + 1.5*mapSize*penalty + min(self.depth,mapSize*mapSize/5)
...
def main():
    ...
    B_init = Board(board_init, center, center, 0, 0, board_goal)
    execute(B_init)
    if (solved):
